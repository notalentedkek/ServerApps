...
counter = 0
lock = threading.Lock()  # Обычная блокировка
ITERATIONS = 100_000
    ...
    def run(self):
        ...
        for _ in range(ITERATIONS):
            with lock:  # Защищаем критическую секцию
                counter += 1  # Атомарная операция под блокировкой

class DecrementThread(threading.Thread):
    def run(self):
        global counter
        for _ in range(ITERATIONS):
            with lock:  # Та же блокировка для обеих операций
                counter -= 1
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python Test3B.py <num_increment_threads> <num_decrement_threads>")
        sys.exit(1)
